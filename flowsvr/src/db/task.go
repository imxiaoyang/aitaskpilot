...
// CreateTable 创建任务信息表（需要任务类型，已知表的position）
...
func (p *Task) GetTaskList(db *gorm.DB,
	...
	err := db.
		...
		Where("order_time < ?", time.Now().Unix()).
		Where("status = ?", status).
		Order("order_time").
		Limit(limit).
		Find(&taskList).Error
...
func (p *Task) SetContext(db *gorm.DB, taskId, context string) error {
	...
	taskType, pos := p.getTablePosFromTaskId(taskId)
	//dic作为update的参数，update将dic的key作为字段名，value作为字段值
	err := db.Table(p.getTableName(taskType, pos)).Where("task_id = ?", taskId).Updates(dic).Error
	if err != nil {
		return err
	}
...
// GetAssignTasksByOwnerStatus 在指状态定任务列表中获取对应归宿和的任务列表
